...
export const promoterApi = {
  ...
  getPromoters: async (params: PromoterListParams = {}): Promise<PromoterListResponse> => {
  ...
  createPromoter: async (payload: PromoterMutationPayload): Promise<void> => {
    try {
      await apiClient.post('/admin/promoter', buildPromoterBodyWithPhoneNumber(payload));
    } catch (firstError) {
      ...
      try {
        await apiClient.post('/admin/promoter', buildPromoterBodyForUpdate(payload));
      } catch (secondError) {
        throw secondError;
      }
    }
  ...
  deletePromoter: async (promoterId: string): Promise<{ success: boolean; message: string }> => {
